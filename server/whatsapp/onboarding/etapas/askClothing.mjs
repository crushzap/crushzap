...
export async function handle(ctx) {
  ...
  const outMsg = await prisma.onboardingMessage.create({ data: { conversationId: conv.id, userId: user.id, personaId: persona.id, step: 'askCommModeFinal', direction: 'out', type: 'text', content: bodyExplain, status: 'queued', metadata: { buttons } } })
  const result = await sendWhatsAppButtons(sendId, phone, bodyExplain, buttons)
